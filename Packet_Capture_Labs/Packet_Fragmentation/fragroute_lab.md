...
You will see exactly three packets: two attack fragments and one **RST|ACK** from the victim.

2. Click Fragment 1. Expand **Transmission Control Protocol** - Wireshark will show partial or un-decodable TCP because the fragment is only 8 bytes. The **Flags** field reads `0x00` or cannot be decoded. This is exactly what a stateless IDS sees.

3. Click Fragment 2. Expand **Internet Protocol Version 4**:
   - **Fragment Offset: 1** -> byte 8. This fragment contains the rest of the TCP header
   - Expand **Transmission Control Protocol**: the full SYN flags are now visible

4. Click the RST|ACK from `192.168.20.10`. Expand **TCP** and verify:
   - **Acknowledgment Number (raw): 0xDEAD1235** = TINY_ISN + 1 = 3,735,344,692 + 1
...
Despite its evasion capability, malicious IP fragmentation leaves several detectable artifacts that a well-configured passive sensor can identify. The key indicators are: abnormally small fragment sizes (especially 8-byte first fragments), unusually sequential IP Identification values across attack flows, a disproportionate number of fragments from a single source IP, inter-arrival times characteristic of tool-generated bursts rather than real MTU constraints, and the presence of overlapping fragments (which have no legitimate use and are flagged by RFC 1858 and RFC 3128).

### Steps

1. **Abnormal fragment sizes - tiny first fragment:**
...
Click Fragment 1. Read **Total Length: 28** (20-byte IP header + 8-byte data payload). Any non-final fragment with an IP data payload of exactly 8 bytes is a strong indicator of a deliberate tiny-fragment attack - legitimate MTU-imposed fragmentation uses ~1480-byte data chunks on Ethernet.

2. **Sequential IP Identification values:**

Apply: 

```
ip.src == 192.168.20.99 and (ip.flags.mf == 1 or ip.frag_offset > 0)
```

Sort by Time. In the **Identification** column the attack IP IDs arrive as: `0x4141`, `0x4242`, `0x4343`, `0x4444`, `0x4545`, `0x4646` - six consecutively spaced values. A real OS generates IDs pseudo-randomly; tool-generated sequential IDs are a clear signature.

3. **Fragment density from a single source:**
Apply:

```
ip.src == 192.168.20.99
```

In **Statistics -> Conversations -> IPv4** note the packet counts from `192.168.20.99`. Compare to `ip.src == 192.168.20.50` - the background noise host generates zero intentional attack fragments.

4. **Overlapping fragment expert warning:**
...
Look for a yellow warning icon (Expert Information) on Fragment B. In **Analyze -> Expert Information** filter for **Warning** to see all fragmentation anomalies captured by Wireshark's heuristics.

5. **MF mid-chain fragments with anomalous size:**
...
These are fragments that are neither the first nor the last piece - they exist only inside multi-fragment chains. The attacker's mid-chain fragments are 8 or 24 bytes; legitimate large-ping fragmentation generates ~1480-byte chunks.

6. **Fragment storms - rapid repeat of same IP ID:**
...
In **Statistics -> I/O Graph** add a filter `ip.id == 0x4444` with a 10 ms interval. All 7 packets appear as a spike in a single time bin - a burst signature inconsistent with natural retransmission or routing-imposed fragmentation.
