...
## Section 1 — Overview: What WireGuard Looks Like on the Wire
...
Wireshark ships with a built-in WireGuard dissector (`wireguard` protocol) that parses these fields by name.
...
```
   wireguard
```
   If the dissector is not loaded, install Wireshark ≥ 3.4.
...
## Section 2 — Handshake Initiation (Type 1)
...
2. Click the first initiation. Expand **WireGuard** in the detail pane. Confirm:
   - **Type**: `Handshake Initiation (1)`
   - **Sender Index**: note this value — you will verify it appears as the Receiver Index in the response.
...
4. Compare the **Ephemeral** field across all six initiations. Every value is different — fresh ephemeral keys per handshake is what enables forward secrecy.

---

## Section 3 — Handshake Response (Type 2)
...
## Section 4 — Transport Data Packets (Type 4)
...
2. Click any transport packet. Expand **WireGuard**:
   - **Receiver Index**: compare to the index values from Section 3 — you can identify which session this packet belongs to without decrypting it.
...
## Section 7 — What Is and Is Not Visible

### Background

This section synthesises what a passive observer can and cannot determine. In a CTF context this maps directly to questions like "what IP is behind the VPN", "what protocol is tunnelled", or "how many sessions occurred".
